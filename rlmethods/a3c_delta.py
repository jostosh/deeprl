...
from deeprl.approximators.nn_delta import ActorCriticNN
...
class A3CAgent(object):
    ...
    def _train(self):
        ...
        while T < hyperparameters.T_max:
            ...
            while not terminal_state and self.t - t_start != hyperparameters.t_max:
                ...
                if net.recurrent:
                    lstm_states.append(deepcopy(net.lstm_state_numeric))
            ...
            for i in reversed(range(batch_len)):
                ...
                fdict = {
                    net.inputs: [states[i]],
                    net.actions: [actions[i]],
                    net.advantage_no_grad: np.asarray([n_step_target - values[i]]).reshape((1, 1)),
                    net.n_step_returns: np.asarray([n_step_target]).reshape((1, 1))
                }
                if net.recurrent:
                    fdict[net.initial_state] = lstm_states[i]
                    fdict[net.n_steps] = [1]
                if net.frame_prediction:
                    if i == hyperparameters.t_max - 1:
                        fdict[net.frame_target] = [self.last_state[-1, :, :]]
                    else:
                        fdict[net.frame_target] = [states[i + 1][-1, :, :]]
                session.run(net.accumulate_delta, feed_dict=fdict)

            session.run(net.update_from_accumulates, feed_dict={net.optimizer.learning_rate: current_lr})
            session.run(net.reset_delta)
            ...
            if T - last_checkpoint > hyperparameters.evaluation_interval \
                ...
                saver.save(session, weights_path, global_step=T)
    ...
    def evaluate(self, num_episodes):
        ...
        while episode_idx < num_episodes:
            ...
            if EXPORT_EMBEDDINGS and t % 100 == 0:
