...
class ActorCriticNN(object):
    ...
    def __init__(self, num_actions, agent_name, optimizer, hyper_parameters, global_network=None, distributed=False):
        ...
        with tf.name_scope(agent_name):
            ...
            self.optimizer.set_global_theta(self.theta)
    ...
    def get_action(self, state, session, lstm_state_numeric):
        """
        ...
        if self.recurrent:
            pi, self.lstm_state_numeric = session.run(
                ...
                feed_dict={
                    self.inputs: [state],
                    self.initial_state: lstm_state_numeric,
                    self.n_steps: [1]
                }
            )
        else:
    ...
    def get_value(self, state, session, lstm_state_numeric):
        """
        ...
        if self.recurrent:
            ...
            return session.run(self.value, feed_dict={self.inputs: [state], self.initial_state: lstm_state_numeric,
                                                      self.n_steps: [1]}
                               )[0]#[0]
    ...
    def get_value_and_action(self, state, session, lstm_state_numeric):
        """
        Returns the action and the value
        """
        if self.recurrent:
            value, pi, lstm_state_numeric = session.run(
                [
    ...
    def update_params(self, actions, states, lr, last_state, session, rewards, initial_return,
                      lstm_first_state_since_update, lstm_state_numeric, upper_limits=None, lower_limits=None):
        """
        ...
        if self.recurrent:
            ...
            fdict = {
                ...
                self.inputs: states,
                self.initial_state: lstm_first_state_since_update,
            ...
            lstm_first_state_since_update = deepcopy(lstm_state_numeric)
        else:
            fdict = {
                self.actions: actions,
                self.inputs: states,
