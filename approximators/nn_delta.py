...
class ActorCriticNN(object):
    ...
    def build_delta_accumulator(self):
        self.delta_theta = [tf.Variable(tf.zeros(shape=var.get_shape().as_list(), dtype=tf.float32))
                            for var in self.theta]
        self.reset_delta = [tf.assign(t, t * 0.) for t in self.delta_theta]
        grads = [tf.clip_by_norm(grad, 40.0) for grad in tf.gradients(self.loss, self.theta)]
        self.accumulate_delta = [tf.assign_add(d_t, g) for d_t, g in zip(self.delta_theta, grads)]
        self.update_from_accumulates = self.optimizer.build_update_from_grads(self.delta_theta)
